/* ============================================
   FAKECC S.A.C. - PROYECTOS DINÁMICOS
 ...
 *     rubro: 'Tipo de obra (Obras Viales, Saneamiento, Edificaciones, etc)',
 *     monto: 'S/ X,XXX.XX',
...
function renderProjects() {
    ...
    if (!CONFIG || !CONFIG.projects || CONFIG.projects.length === 0) {
        projectsGrid.innerHTML = '<p style="text-align: center; color: #5C6470; grid-column: 1/-1;">No hay proyectos disponibles.</p>';
        return;
    }
...
function createProjectCard(project, index) {
    ...
    const montoHtml = project.monto && !project.monto.includes('0.00')
        ? `<span class="project__monto">${project.monto}</span>`
        : '';
...
function getProjectsStats() {
    ...
    const promedioMonto = montos.length ? montoTotal / montos.length : 0;
...
console.log(`
╔════════════════════════════════════════════════════════════╗
║  FAKECC S.A.C. - PROYECTOS DINÁMICOS CARGADOS             ║
╠════════════════════════════════════════════════════════════╣
║  Funciones disponibles en consola:                         ║
║                                                            ║
║  • filterProjectsByCategory('Obras Viales')               ║
║  • searchProjects('saneamiento')                          ║
║  • getProjectsStats()                                     ║
